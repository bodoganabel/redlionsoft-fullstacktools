...
export const REGEX_VALID_PHONE = /^\+\s*(\d\s*){1,14}$/;

export const VALIDATOR_IS_EMAIL = (message = 'Invalid email') => (value: string) => { if (REGEX_VALID_EMAIL.test(value) === false) return message; }

export const VALIDATOR_IS_PHONE = (message = 'Invalid phone') => (value: string) => { if (REGEX_VALID_PHONE.test(value) === false) return message; }
