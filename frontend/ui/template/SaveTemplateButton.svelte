  ...
  async function handleSave() {
    popupInput({
      ...
      onSave: async (templateName: string) => {
        const existingTemplate = templates.find((template) => template.resourceId === templateName);

        if (existingTemplate) {
          popup({
            id: 'confirm-overwrite',
            title: 'Template already exists',
            message: 'A template with this name already exists. Do you want to overwrite it?',
            onAccept: async () => {
