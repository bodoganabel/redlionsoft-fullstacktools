...
export function createFormContext() {
    ...
    const formContext: IFormContext = { errors, touched, validators, inputNodes, validateAll: () => false };
    formContext.validateAll = () => { return validateAll(formContext) }
    return formContext;
}
...
export function validateAll(formContext: IFormContext) {
    ...
    inputNodes.forEach(node => {
        ...
        if (name) {
            if (!validate(name, value, formContext)) { allValid = false; }
        }
    })
    return allValid;
}
...
function validate(name: string, value: any, formContext: IFormContext): boolean {
    ...
    const inputNode = formContext.inputNodes.find(node => node.getAttribute('name') === name);
    if (inputNode) {
...
export function formField(node: HTMLElement, params: { formContext: IFormContext, validators: ((value: any) => string | undefined)[] }) {
    ...
    console.log('get(formContext.validators):');
    console.log(get(formContext.validators)
    );
