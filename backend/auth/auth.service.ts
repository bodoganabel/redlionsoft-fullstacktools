 ...
 */
type TBareMinimumUserType = { email: string; password: string; permissions: string[] } & {
	[key: string]: any;
};

export class AuthService<TUserServer extends TBareMinimumUserType, TUserClient> {
	private jwt: JWT;
	...
	public async login(
		...
		try {
			...
			try {
				const clientUser: TUserClient = this.parse_serverUserTo_clientUser(user as TUserServer);
